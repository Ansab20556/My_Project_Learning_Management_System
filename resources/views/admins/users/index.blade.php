            ...
            <tbody>
                @foreach ($users as $user)
                    <tr>
                        <td>{{ $user->name }}</td>
                        <td>{{ $user->email }}</td>
                        <td>{{ $user->isSuperAdmin() ? 'مدير عام' : ($user->isAdmin() ? 'استاذ' : 'طالب') }}</td>
                        <td>
                            <form class="ml-4 form-inline" method="POST" action="{{ route('admin.users.update', $user) }}" style="display:inline-block">
                                @method('patch')
                                @csrf
